...
// ---- a tiny in-memory stand-in for the pg client (no real DB touched) --------
// It understands exactly the SQL shapes embedSource issues, so the tests prove
// the real control flow (probe -> only-NULL select -> batched txn -> resume).
function makeClient(chunks, opts = {}) {
  const state = {
...
function makeProvider({ dim = 3, fail } = {}) {
  const calls = [];
  return {
    calls,
    modelId: () => 'mock-model',
...
const cfg = { embeddingBatchSize: 2, embeddingConcurrency: 1 };
const connectTo = (client) => async () => client;

function nChunks(n, preEmbedded = 0) {
  const arr = [];
  for (let i = 1; i <= n; i++) {
